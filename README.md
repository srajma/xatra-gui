...
- [x] Our current new map and landing page workflow causes an unnecessary proliferation of maps, because each "New Map" click generates a fresh incrementing name (new_map_1, new_map_2…) even if the previous scratch map was never published. Fixed: `/maps/default-name` now reuses the user's most recently updated unpublished new_map_* artifact instead of always incrementing.
